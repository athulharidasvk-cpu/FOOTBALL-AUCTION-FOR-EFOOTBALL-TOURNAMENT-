...
(function () {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #${MODAL_ID}.pls-25d-mode .pls-canvas-wrapper {
        ...
        isolation: isolate;
        background:
          radial-gradient(ellipse at 50% 42%, rgba(20,72,42,.95) 0%, rgba(5,27,20,.98) 48%, #020914 100%);
        overflow: hidden;
      }
      ...
      #${MODAL_ID}.pls-25d-mode .pls-canvas-wrapper::before {
        ...
        background:
          ...
          linear-gradient(180deg,rgba(8,56,34,.75),rgba(1,12,9,.98));
      ...
      #${MODAL_ID}.pls-25d-mode .pls-canvas-wrapper::after {
        ...
        pointer-events: none;
        background:
          linear-gradient(180deg,rgba(255,255,255,.035),transparent 18%,transparent 78%,rgba(0,0,0,.22)),
      ...
      #${MODAL_ID}.pls-25d-mode #${CANVAS_ID} {
        ...
        transform: perspective(1050px) rotateX(14deg) scale(1.045) translateY(10px);
        filter: saturate(1.08) contrast(1.04) drop-shadow(0 24px 22px rgba(0,0,0,.46));
        image-rendering: auto;
      }

      #${MODAL_ID}.pls-25d-mode .pls-canvas-wrapper::marker { display:none; }
      ...
      .pls25d-stadium-glow {
        ...
        pointer-events:none;
        z-index:0;
        background:radial-gradient(ellipse,rgba(255,255,255,.16),rgba(56,189,248,.06) 35%,transparent 72%);
        filter:blur(12px);
      }
      ...
      .pls25d-side-depth {
        ...
        pointer-events:none;
        opacity:.7;
        background:linear-gradient(180deg,rgba(56,189,248,.05),rgba(2,6,12,.5));
      }
      ...
      #${MODAL_ID}.pls-25d-mode .pls-controls-overlay {
        z-index: 10;
      }

      #${MODAL_ID}.pls-25d-mode .pls-commentary-bar {
        z-index: 12;
        box-shadow:0 8px 24px rgba(0,0,0,.45),0 0 18px rgba(56,189,248,.08);
        backdrop-filter:blur(8px);
      }

      #${MODAL_ID}.pls-25d-mode .pls-event-banner {
        z-index: 15;
      }
      ...
      @media (max-width: 700px) {
        ...
        #${MODAL_ID}.pls-25d-mode #${CANVAS_ID} {
          transform: perspective(820px) rotateX(10deg) scale(1.035) translateY(7px);
        }
        .pls25d-depth-badge { top:8px; right:8px; font-size:8px; }
      }

      @media (prefers-reduced-motion: reduce) {
        #${MODAL_ID}.pls-25d-mode #${CANVAS_ID} { transition:none !important; }
      }
    `;
  ...
  function decorate() {
    ...
    if (!wrapper.querySelector(".pls25d-stadium-glow")) {
      ...
      glow.className = "pls25d-stadium-glow";
      wrapper.appendChild(glow);
    }

    if (!wrapper.querySelector(".pls25d-depth-badge")) {
      const badge = document.createElement("div");
      badge.className = "pls25d-depth-badge";
      badge.textContent = "2.5D MATCH ENGINE";
      wrapper.appendChild(badge);
    }

    if (!wrapper.querySelector(".pls25d-side-depth.left")) {
      const left = document.createElement("div");
      left.className = "pls25d-side-depth left";
      const right = document.createElement("div");
      right.className = "pls25d-side-depth right";
      wrapper.append(left, right);
    }

    // Subtle parallax from pointer movement. This changes only the presentation layer.
    if (!wrapper.dataset.parallax25d) {
      ...
      wrapper.addEventListener("pointerleave", () => {
        canvas.style.setProperty("--pls25dx", "0deg");
        canvas.style.setProperty("--pls25dy", "0deg");
      }, { passive: true });
    }

    // Reapply the presentation class if another UI layer changes modal classes.
    return true;
  }
