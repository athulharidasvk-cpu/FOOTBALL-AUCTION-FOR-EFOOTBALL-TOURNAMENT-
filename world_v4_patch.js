...
world.updateEconomy = function(w, clubName, data) {
  ...
  if (data.capacity !== undefined) {
    const cap = Math.max(c.stadium.capacity, Math.round(Number(data.capacity)||c.stadium.capacity));
    const cost = Math.max(0, Math.round((cap-c.stadium.capacity)/5000*4));
    if (cost > c.cash) return {error:`Need ₹${cost}M for stadium expansion.`};
    c.cash -= cost; c.stadium.capacity = cap;
  }
...
world.sellPlayer = function(w,clubName,playerId,asking){
  ...
  p.status='listed';
  w.news.unshift({id:`sell_${Date.now()}`,season:w.season,type:'Transfer Market',text:`${p.name} is listed by ${c.name} for ₹${p.askingPrice}M.`});
  return p;
};
